...
def main():
    ...
    for seed in (5301,5302,5303):
        pr=rows(plain_root/f"l3_seed{seed}"/"evaluation_history.csv");ir=rows(iw_root/f"seed{seed}"/"evaluation_history.csv")
        curves[str(seed)]={"plain":[summarize(nearest(pr,s)) for s in STEPS],"iwsc":[summarize(nearest(ir,s)) for s in STEPS]}
        mechanisms[str(seed)]=mechanism(iw_root/f"seed{seed}"/"optimization_metrics.jsonl")
        summary=json.loads((iw_root/f"seed{seed}"/"run_summary.json").read_text(encoding="utf-8"));complete &= int(summary.get("sampled_steps",-1))==3000000
    ...
    gate=(complete and means["delta_average_waves"]>=.15 and sum(x["delta_average_waves"]>0 for x in paired)>=2 and
          means["delta_W1"]>=-.05 and means["delta_Q2"]>0 and means["delta_Q3"]>0 and
          max(means["delta_Q2"],means["delta_Q3"])>=.05)
    print(json.dumps({"offline_only":True,"all_three_complete":complete,"curves":curves,"mechanism_metrics":mechanisms,"paired_deltas":paired,"mean_deltas":means,
