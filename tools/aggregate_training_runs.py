...
def main() -> None:
    ...
    run_dirs = [path if path.is_absolute() else PROJECT_ROOT / path
                for path in args.run_dirs]
    output_dir = (
        args.output_dir if args.output_dir.is_absolute()
        else PROJECT_ROOT / args.output_dir
    )
    histories = [read_history(path) for path in run_dirs]
    ...
    write_rows(output_dir / "figure8_data.csv", figure8)
    write_rows(output_dir / "figure9_data.csv", figure9)
    print(f"wrote {len(common_steps)} common evaluation points to {output_dir}")
