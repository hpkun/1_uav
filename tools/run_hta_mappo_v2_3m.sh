...
if not torch.cuda.is_available():
    raise SystemExit("CUDA is mandatory for HTA-MAPPO V2 development")
PY

for seed in 5301 5302 5303; do
  output="outputs/dev_hta_mappo_v2_3m/seed${seed}"
  if [[ -e "$output" ]]; then
  ...
  python -u algorithm/train_modular_mappo.py \
    ...
    --algorithm-config configs/dev_hta_mappo_v2_3m.yaml \
    --output-dir "$output" \
    --device cuda \
    --seed "$seed" \
