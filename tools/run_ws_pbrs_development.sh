...
run_one(){ local method="$1" seed="$2" config="$3" dir="outputs/dev_ws_pbrs/${method}_seed${seed}"; [[ ! -e "$dir" ]] || { echo "Refusing non-fresh $dir" >&2; exit 1; }; python -u algorithm/train_modular_mappo.py --env-config configs/persistent_wave_v2_environment.yaml --algorithm-config "$config" --output-dir "$dir" --device cuda --seed "$seed" --num-envs 24 --total-sampled-steps 900000 > "${dir}_nohup.log" 2>&1; }
