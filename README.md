...
Environment V2.3**. It uses the motion equations, action increments, combat
...
python -u scripts/train_madsac.py \
  ...
  --num-envs 24 \
  --output-dir outputs/madsac_v2_3
```
...
python -u scripts/train_mappo.py \
  ...
  --num-envs 24 \
  --output-dir outputs/mappo_v2_3_impl_v2_8m_seed2023
```
...
explicit diagnostic path and cannot resume formal training. Replay contents are
not checkpointed, so even a compatible MADSAC resume restarts replay collection
and episodes. Both algorithms also save `best_eval.pt`, selected
lexicographically by win rate, team return, then lower Red loss.
