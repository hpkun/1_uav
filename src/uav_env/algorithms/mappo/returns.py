...
def compute_gae(
    ...
    truncated: NDArray[np.bool_], terminal_values: NDArray[np.float32],
    truncation_bootstrap_mask: NDArray[np.float32], gamma: float, gae_lambda: float,
) -> tuple[NDArray[np.float32], NDArray[np.float32]]:
    """Return advantages/returns for arrays shaped ``[T,E,A]`` and values ``[T+1,E,A]``."""

    if (
        values.shape[0] != rewards.shape[0] + 1
        or terminated.shape != rewards.shape[:2]
        or truncated.shape != rewards.shape[:2]
        or terminal_values.shape != rewards.shape
        or truncation_bootstrap_mask.shape != truncated.shape
    ):
    ...
    for step in range(rewards.shape[0] - 1, -1, -1):
        ...
        allowed_terminal_bootstrap = terminal_values[step] * truncation_bootstrap_mask[step, :, None]
        bootstrap = np.where(term, 0.0, np.where(trunc, allowed_terminal_bootstrap, values[step + 1]))
