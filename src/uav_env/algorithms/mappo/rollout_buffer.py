...
class RolloutBuffer:
    ...
    def insert(self, actions, log_probs, values, rewards, terminated, truncated, actor_masks, critic_masks,
               next_obs, next_states, next_available, terminal_values) -> None:
        ...
        self.terminal_values[i]=terminal_values; self.step+=1
    ...
    def finish(self, last_values, gamma: float, gae_lambda: float) -> None:
        ...
        self.next_values[self.truncated]=self.terminal_values[self.truncated]
        self.advantages,self.returns=compute_gae(self.rewards,self.values,self.terminated,self.truncated,self.terminal_values,gamma,gae_lambda)
