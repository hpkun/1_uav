"""Plot deterministic 3D tracks for a MAPPO checkpoint."""
...
def rollout(actor: MAPPOTrainer, environment_config: dict, seed: int,
    ...
    def append(step: int, side: str, states, include) -> None:
        if not capture:
            return
        for index, (state, keep) in enumerate(zip(states, include)):
            if keep:
                tracks[(side, index)].append({
                    ...
                    "x_m": float(state.x),
                    "y_m": float(state.y),
                    "altitude_m": float(state.altitude),
...
def representative_seed(actor: MAPPOTrainer, environment: dict, seed_base: int,
                        episodes: int) -> int:
    rows = [
        rollout(actor, environment, seed)[0]
        for seed in range(seed_base, seed_base + episodes)
    ]
    winning = [row for row in rows if row["termination_reason"] == "red_win"]
    candidates = winning or rows
    median_return = float(np.median([row["team_return"] for row in candidates]))
    return min(
        candidates,
        key=lambda row: abs(row["team_return"] - median_return),
    )["seed"]


def plot_tracks(seed: int, summary: dict, tracks: dict, output: Path) -> None:
    ...
    all_points = []
    for side, colors, linestyle in (
        ("red", red_colors, "-"), ("blue", blue_colors, "--")
    ):
        for index in range(4):
            ...
            if not points:
                continue
            xyz = np.asarray([
                [p["x_m"], p["y_m"], p["altitude_m"]] for p in points
            ]) / 1000.0
            all_points.append(xyz)
            axis.plot(
                xyz[:, 0], xyz[:, 1], xyz[:, 2], linestyle,
    ...
    axis.plot(
        5.0 * np.cos(angle), 5.0 * np.sin(angle), np.zeros_like(angle),
    ...
    axis.set_title(
        f"MAPPO checkpoint deterministic 4v4 trajectory\n"
        f"seed={seed} | {summary['termination_reason']} "
        f"| return={summary['team_return']:.2f} "
        f"| Red loss={summary['red_losses']} "
        f"| Blue loss={summary['blue_losses']} "
...
def main() -> None:
    ...
    seed = (
        ...
        else representative_seed(actor, environment, args.seed_base, args.episodes)
    )
    summary, tracks = rollout(actor, environment, seed, capture=True)
    output_dir = resolved(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    stem = f"mappo_checkpoint_3d_trajectory_seed_{seed}"
    png = output_dir / f"{stem}.png"
    csv_path = output_dir / f"{stem}.csv"
    plot_tracks(seed, summary, tracks, png)
    with csv_path.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(
            stream,
            fieldnames=("side", "aircraft", "step", "x_m", "y_m", "altitude_m", "alive"),
        )
        writer.writeheader()
