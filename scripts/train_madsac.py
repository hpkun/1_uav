...
def main()->None:
    p=argparse.ArgumentParser(); p.add_argument("--device",choices=["cpu","cuda"]); p.add_argument("--seed",type=int); p.add_argument("--total-sampled-steps",type=int); p.add_argument("--num-envs",type=int); p.add_argument("--output-dir"); p.add_argument("--resume"); p.add_argument("--env-config",default="configs/combat_environment.yaml"); p.add_argument("--algorithm-config",default="configs/madsac.yaml"); p.add_argument("--smoke",action="store_true")
    ...
    env_cfg=yaml.safe_load(env_path.read_text(encoding="utf-8")); alg_path=Path(args.algorithm_config); alg_path=alg_path if alg_path.is_absolute() else root/alg_path; alg_cfg=yaml.safe_load(alg_path.read_text(encoding="utf-8"))
