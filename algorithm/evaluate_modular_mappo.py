...
def main():
 ...
 checkpoint=resolved(a.checkpoint);state=torch.load(checkpoint,map_location="cpu",weights_only=False);extra=state.get("extra",{});config=extra.get("algorithm_config");protocol_complete=True
 ...
 if protocol_complete:trainer.load(checkpoint)
 else:
  trainer.actor.load_state_dict(state["actor"]);trainer.critic.load_state_dict(state["critic"]);trainer.popart.load_state_dict(state.get("popart",{}),strict=False);trainer.sampled_steps=int(state.get("sampled_steps",0))
